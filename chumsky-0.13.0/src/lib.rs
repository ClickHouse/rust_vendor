...
#![allow(clippy::style, clippy::useless_format, clippy::type_complexity)]
...
pub enum DefaultExpected<'a, T> {
    ...
    SomethingElse,
    /// There was no input that could have satisfied the parser.
    NothingElse,
...
impl<T> DefaultExpected<'_, T> {
        ...
        T: Clone,
    {
        match self {
            ...
            Self::NothingElse => DefaultExpected::NothingElse,
...
pub trait Parser<'src, I: Input<'src>, O, E: ParserExtra<'src, I> = extra::Default> {
    ...
    ///
    /// Note that this overrides any error messages manually specified through combinators like [`Self::try_map_with`]
    // TODO: Example
        ...
        E::Error: LabelError<'src, I, L>,
    {
        Labelled {
            ...
            label,
            is_context: false,
            debug_info_override: Default::default(),
        }
    }
    ...
    /// encountered).
    ///
    /// In general, this method should be avoided except in cases where all possibilities that the parser might produce can
    /// be parsed by using [`FromStr`] without producing an error.
    ///
...
impl IterParserDebug {
    #[inline(always)]
    pub(crate) fn new(#[allow(unused_variables)] nonconsumption_is_ok: bool) -> Self {
        Self {
            #[cfg(debug_assertions)]
    ...
    E: ParserExtra<'src, I>,
{
    ...
    /// Collect this iterable parser into a container that implements [`FromIterator`].
    ///
    ...
    /// ```
    #[cfg_attr(debug_assertions, track_caller)]
    fn collect<C: FromIterator<O>>(self) -> Collect<Self, O, C>
    where
        Self: Sized,
    {
        Collect {
...
mod tests {
    ...
    */
    ...
    #[test]
    fn expected_nothing() {
        fn parser<'i>() -> impl Parser<'i, &'i str, &'i str, extra::Err<Rich<'i, char>>> {
            just("foo").contextual().configure(|_, _| false)
        }

        let mut errs = parser().parse("foo").into_errors();
        assert_eq!(errs.len(), 1);
        // The parser is unsatisifiable, so nothing should be expected
        assert_eq!(errs.remove(0).expected().len(), 0);
    }

    // Regression test for https://codeberg.org/zesterer/chumsky/issues/985
    #[test]
    fn labelled_context_validate() {
        use crate::input::MappedInput;

        type Span = SimpleSpan;
        type Token = Spanned<char, Span>;
        type ParserInput<'tokens> = MappedInput<'tokens, char, Span, &'tokens [Token]>;
        type ParseError<'tokens> = extra::Err<Rich<'tokens, char, Span>>;

        fn parser<'tokens>(
        ) -> impl Parser<'tokens, ParserInput<'tokens>, Vec<bool>, ParseError<'tokens>> {
            let single = just("a")
                .ignore_then(one_of("bc").spanned())
                .validate(|b_or_c, _, emitter| {
                    let Spanned { inner, span } = b_or_c;
                    match inner {
                        'b' => true,
                        'c' => {
                            emitter.emit(Rich::custom(span, "expected 'b' but found 'c'"));
                            false
                        }
                        _ => unreachable!(),
                    }
                })
                .labelled("SINGLE PARSER")
                .as_context();
            single.repeated().collect()
        }

        // Conceptually represents the source string "ac ab ac ad"
        let tokens = vec![
            Token {
                inner: 'a',
                span: Span::from(0..1),
        ...
        let eoi = Span::from(8..8);

        let (output, errors) = parser()
            .parse(tokens.split_spanned(eoi))
            .into_output_errors();

        assert_eq!(output, Some(vec![false, true, false]));
        assert_eq!(errors.len(), 2);
        if let [custom_1, custom_2] = &errors[..] {
            let custom_1_contexts: Vec<_> = custom_1.contexts().collect();
            assert_eq!(*custom_1.span(), Span::from(1..2));
            assert_eq!(custom_1_contexts.len(), 1);
            assert_eq!(*custom_1_contexts[0].1, Span::from(0..2));

            let custom_2_contexts: Vec<_> = custom_2.contexts().collect();
            assert_eq!(*custom_2.span(), Span::from(7..8));
            assert_eq!(custom_2_contexts.len(), 1);
            assert_eq!(*custom_2_contexts[0].1, Span::from(6..8));
        } else {
            unreachable!();
        }
    }
