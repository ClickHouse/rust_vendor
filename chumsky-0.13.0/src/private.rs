...
pub trait Mode {
    ...
    /// Given an iterator of outputs, collect them into a container. The iterator will always be
    /// consumed.
    fn from_iter<T, C: FromIterator<T>>(
        iter: impl Iterator<Item = Self::Output<T>>,
    ) -> Self::Output<C>;
