    ...
    A: Parser<'src, I, O, E>,
{
    ...
    fn go<M: Mode>(&self, inp: &mut InputRef<'src, '_, I, E>) -> PResult<M, O> {
        ...
        if res.is_err() {
            // Remember the result of the parse at this location
            let alt = inp.errors.alt.clone();
    ...
    A: Parser<'src, I, O, E>,
{
    ...
    fn next<M: Mode>(
        ...
        #[allow(unused_variables)] debug: IterParserDebug,
    ) -> IPResult<M, O> {
...
/// See [`IterParser::count`].
pub struct Count<A, O> {
    pub(crate) parser: A,
    #[allow(dead_code)]
    pub(crate) phantom: EmptyPhantom<O>,
}

impl<A: Copy, O> Copy for Count<A, O> {}
impl<A: Clone, O> Clone for Count<A, O> {
    fn clone(&self) -> Self {
        Self {
            parser: self.parser.clone(),
            phantom: EmptyPhantom::new(),
...
impl<'src, I, O, E, A> Parser<'src, I, usize, E> for Count<A, O>
where
    I: Input<'src>,
    E: ParserExtra<'src, I>,
    A: IterParser<'src, I, O, E>,
{
    ...
    #[inline(always)]
    fn go<M: Mode>(&self, inp: &mut InputRef<'src, '_, I, E>) -> PResult<M, usize> {
        let mut iter_state = self.parser.make_iter::<Check>(inp)?;
        let mut n = M::bind(|| 0);
        loop {
            match self
                .parser
                .next::<Check>(inp, &mut iter_state, IterParserDebug::new(false))?
            {
                Some(()) => n = M::map(n, |n| n + 1),
                None => break Ok(n),
            }
        }
    }
    ...
    C: FromIterator<O>,
{
    ...
    fn go<M: Mode>(&self, inp: &mut InputRef<'src, '_, I, E>) -> PResult<M, C> {
        ...
        let c = M::from_iter(core::iter::from_fn(|| {
            match self
                .parser
    ...
    A: Parser<'src, I, O, E>,
{
    ...
    fn go_cfg<M: Mode>(
        ...
        if cfg {
            ...
            let err_span = inp.span_since(&before);
            inp.add_alt([DefaultExpected::NothingElse], None, err_span);
            Err(())
        }
    }
}
